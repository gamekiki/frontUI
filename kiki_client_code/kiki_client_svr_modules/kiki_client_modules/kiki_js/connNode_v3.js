...
//			address : "localhost",
		...
		connSocket : function() {
			...
			var mURL = "ws://"+this.getAddress.address+":"+"3333";
...
//					this.sendMessage('check', 'N', 1, 'null');		// '{"sender":"client"}'
	  ...
	  uploadImage : function(imgData) {
			...
			var mURL = "http://"+this.getAddress.address+":"+"3000"+"/api/v1/kiki/updateProfile?"+mquery;
...
//			document.addEventListener('backbutton', onBack, false);
	...
	sendLogRequest : function(emitType, logData, callback, appId) {
		...
		window.kiki.logCallback = callback;
		$.getJSON(url,data,callback).promise().done(function(){
			window.kiki.logCallback();
		});
